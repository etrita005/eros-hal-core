...
-   **能力导向设计（Capability Model）**：HAL 暴露设备能力而非设备实例，上层系统通过能力接口访问功能，并支持运行时能力发现与查询，与具体硬件实现解耦。

-   **抽象一致性（Abstraction Consistency）**：同类能力保证接口一致，不同能力允许语义差异。

-   **解耦与可扩展性（Decoupling & Scalability）**：支持不同类型设备（执行器、传感器、IO等），支持插件化 HAL 组件按需加载。

-   **多模式部署支持（Multi-Mode Deployment）**：支持 Passthrough 模式（与 Controller 同进程）、Aggregated 模式（多个 HAL 组件共享独立进程）、Isolated 模式（每个 HAL 组件独立进程），适配不同实时性需求。

-   **生命周期管理（Lifecycle Management）**：HAL 组件实现标准生命周期状态，支持状态监控与恢复。

-   **容错与可恢复性（Fault Tolerance & Recoverability）**：支持组件级故障隔离、热插拔与自动恢复。

-   **安全事件响应能力（Safety Handling）**：HAL 支持响应急停指令、上报异常状态、进入安全状态；安全联动策略由控制器层实现。

-   **通信机制解耦（Transport Agnostic）**：HAL 接口不绑定具体通信机制（如 DDS、共享内存、RPC 等），具体通信方式由部署模式与运行时策略决定。
